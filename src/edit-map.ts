  ...
  initMap(_e: Event): boolean {
    ...
    if (this.polygonData) {
      for (const [pc, item] of Object.entries(this.polygonData)) {
        ...
        google.maps.event.addListener(newPolygon, 'dblclick', (event) => {
          this.removeVertex(event, newPolygon);
        });
        google.maps.event.addListener(newPolygon, 'click', (_event) => {
          this.clickedPolygon(pc, newPolygon);
        });
        google.maps.event.addListener(newPolygon, 'mouseover', (event) => {
          this.mouseoverPolygon(event.domEvent, pc);
        });
        google.maps.event.addListener(newPolygon, 'mouseout', (_event) => {
